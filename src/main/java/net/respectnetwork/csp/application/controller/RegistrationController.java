...
public class RegistrationController {
    ...
    public ModelAndView getDetails(
        ...
        if (!errors) {
            ...
            try {
                ...
                if (!RegistrationManager.validatePhoneNumber(userDetailsForm
                        .getPhone())) {
                ...
                if (!RegistrationManager.validatePassword(userDetailsForm
                        .getPassword())) {
                ...
                if (!userDetailsForm.getPassword().equals(
                        userDetailsForm.getConfirmPassword())) {
                 ...
                 * String errorStr =
                 * "The Email and phone combination has already been used for a cloud name"
                 * ; mv.addObject("error", errorStr); logger.debug(
                 * "Email {} already used by {} , phone {} already used by {} ",
                 * userDetailsForm.getEmail(), existingUsers[1],
                 * userDetailsForm.getMobilePhone(),existingUsers[0]); errors =
                 * true; }
                 */
            } catch (UserRegistrationException e) {
    ...
    public ModelAndView validateCodes(
        ...
        if (request.getParameter("terms") == null
                || !request.getParameter("terms").equalsIgnoreCase("on")) {
        ...
        // Validate Codes
        if (!theManager.validateCodes(sessionIdentifier, validateForm
                .getEmailCode().trim().toUpperCase(), validateForm.getSmsCode()
                .trim().toUpperCase())) {
    ...
    /**
     * Calculate the cost of buying cloudnames, taking cost overrides into
     * account
     */
    static CurrencyCost getCostIncludingOverride(CSPModel cspModel,
            String phoneNumber, int numberOfClouds) {
        ...
        try {
            cspCostOverrideModel = DAOFactory.getInstance()
    ...
    static String formatCurrencyAmount(CurrencyCost currencyCost) {
        return formatCurrencyAmount(currencyCost.getCurrencyCode(),
                currencyCost.getAmount());
    }
    ...
    public ModelAndView registerCloudName(HttpServletRequest request,
        ...
        // one in the
        // properties, then re-direct to referer URL provided in config file.
